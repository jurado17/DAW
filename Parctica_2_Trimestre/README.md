...
```
Para verificar los cambios hacemos lo siguiente. Damos permisos
```bash
sudo ufw enable
```
Y comprobamos que este activo
```bash
sudo ufw status
```
![](/img/c30.png)
...
Cree el directorio para el dominio de la siguiente manera:

```bash
sudo mkdir /var/www/servidor
```
...
```bash
sudo chown -R $USER:$USER /var/www/servidor
```
...
Puede ser conveniente deshabilitar el sitio web predeterminado que viene instalado con Apache. Para deshabilitar el sitio web predeterminado de Apache, escriba lo siguiente:
```bash
